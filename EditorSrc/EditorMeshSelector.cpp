...
EditorMeshSelectorResult EditorMeshSelector::getMeshSelection( int elevation )
{
	...
	if ( md.rowt && md.horizontal && md.rowb ) {
		...
		// Bridge mesh and rotation
			...
			newMeshType = BlockMeshDoubleInnerCorner;
			rotationY = 0.0f;
		}
		else if ( md.br && md.tl ) {
			newMeshType = BlockMeshDoubleInnerCorner;
	...
	// Corner mesh and rotation
		...
		newMeshType = BlockMeshOuterCorner;
		rotationY = 0.0f;
	}
